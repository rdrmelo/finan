...
import { app, auth, db } from "./js/config/firebase.js";
import {
    signOut,
    onAuthStateChanged
...
// document.addEventListener('DOMContentLoaded') removido pois será gerenciado pelo js/main.js


// --- LÓGICA DE AUTENTICAÇÃO REMOVIDA (Movida para js/modules/auth.js) ---
/*
    A lógica de autenticação (setupAuthEventListeners, onAuthStateChanged) foi movida para js/modules/auth.js.
    O initializeAppLogic será chamado pelo js/main.js quando o login ocorrer.
*/

export async function initializeAppLogic(user) {
    currentUser = user; // Atualizar user local
...
function updateWeekCalendar() {
    ...
    container.innerHTML = weekDays.map(date => {
        ...
        return `
                        <div class="text-center p-2 rounded ${isToday ? 'bg-blue-500/20 border border-blue-400/30' :
